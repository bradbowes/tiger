...
var
   ...
   begin
      if decls^.length > 0 then
         expand := make_let_node(decls, n, file_name, line, col)
      else
         expand := n;
   end;
   ...
   begin
      if compound(n) then
         begin
            ...
            line := n^.loc^.line;
            col := n^.loc^.col;
            append_node(decls, make_var_decl_node(v, nil, trans3(n), file_name, line, col));
            reduce := make_simple_var_node(v, file_name, line, col);
         end
      else
         reduce := trans3(n);
   end;
   ...
   begin
      ...
      expand_binary_op := expand(make_binary_op_node(n^.op, left, right, file_name, line, col));
   end;
   ...
   begin
      left := reduce(n^.left);
      expand_unary_op := expand(make_unary_op_node(n^.op, left, file_name, line, col));
   end;
   ...
   begin
      ...
      expand_call := expand(make_call_node(n^.name, args, file_name, line, col));
   end;
   ...
   begin
      cond := reduce(n^.cond);
      expand_if_else := expand(make_if_else_node(cond, trans3(n^.left), trans3(n^.right), file_name, line, col));
   end;
   ...
   begin
      cond := reduce(n^.cond);
      expand_if := expand(make_if_node(cond, trans3(n^.left), file_name, line, col));
   end;
   ...
   begin
      ...
      if compound(left) then
         begin
            ...
            case left^.tag of
               indexed_var_node:
                  left := make_indexed_var_node(lleft, reduce(left^.right), file_name, line, col);
               field_var_node:
                  left := make_field_var_node(lleft, left^.name, file_name, line, col);
            end;
         end
      else
         left := trans3(n^.left);
      right := reduce(n^.right);
      expand_assign := expand(make_assign_node(left, right, file_name, line, col));
   end;
   ...
   begin
      left := reduce(n^.left);
      cond := reduce(n^.cond);
      expand_for := expand(make_for_node(n^.name, left, cond, trans3(n^.right), file_name, line, col));
   end;
   ...
   begin
      left := reduce(n^.left);
      right := reduce(n^.right);
      expand_indexed_var := expand(make_indexed_var_node(left, right, file_name, line, col));
   end;
   ...
   begin
      left := reduce(n^.left);
      expand_field_var := expand(make_field_var_node(left, n^.name, file_name, line, col));
   end;
   ...
   begin
      ...
      while it <> nil do
         begin
            ...
            append_node(list, make_field_node(field^.name, value, field^.loc^.file_name, field^.loc^.line, field^.loc^.col));
            it := it^.next;
         end;
      expand_record := expand(make_record_node(n^.type_name, list, file_name, line, col));
   end;
